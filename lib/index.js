  ...
  request(opts, function(err, response, body) {
    ...
    if(!_.isEmpty(body) && _.isString(body)) {
      ...
      if (opts.url.indexOf("alt=json") !== -1) {
        ...
        try {
          result = parser.toJson(body, { sanitize: false });
...
Spreadsheet.prototype.getSheetId = function(type, callback) {
  ...
  this.request({
    ...
    var entries = _.isEmpty(result.feed.entry) ? [] : _.isArray(result.feed.entry) ? result.feed.entry : [result.feed.entry];

    _this.worksheetNames = _.pluck(entries, 'title');
...
Spreadsheet.prototype.receive = function(options, callback) {
  ...
  _this.request({
    ...
    if (result.feed.author) {
      if (_.isArray(result.feed.author)) {
        info.authors = _.map(result.feed.author, function(author) {
          return { name: author.name, email: author.email };
        });
      } else {
        info.author = result.feed.author;
      }
    }
    var maxRow = 0;

    var matrix = _.reduce(entries, function(memo, entry) {

      if (!entry.title) {
        return memo;
      }

      var coodinate = _this.getCoordinate(entry.title);
      var x = coodinate.x;
      var y = coodinate.y;
      memo[y] = memo[y] || [];
      memo[y][x] = entry.content;
      return memo;
    }, []);
...
Spreadsheet.prototype.getCoordinate = function(cellTitle) {

  var re = new RegExp('[0-9]');
  var index = cellTitle.match(re).index;
  var cells = cellTitle.split('');
  var cell = _.reduce(cells, function(memo, c, _index) {
    if (_index < index) {
      memo.x += c;
    } else {
      memo.y += c;
  ...
  cell.x = this.convertAlphabetToNumber(cell.x);
  cell.y = parseInt(cell.y, 10) - 1;
  return cell;
};

Spreadsheet.prototype.convertAlphabetToNumber = function(string) {

  var num = _.reduce(string.split('').reverse(), function(memo, c, index) {
    memo += parseInt(c, 36) - 10 + index * 26;
    return memo;
  }, 0);
  return num;
};
